...
let imagemFundoEndpoint = null;

window.openModalImagemFundo = function(endpoint) {
    imagemFundoEndpoint = endpoint;
    $('#modalInputArquivo').val('');
    $('#modalInputLink').val('');
    new bootstrap.Modal(document.getElementById('modalImagemFundo')).show();
}

$(document).on('click', '#btnEnviarImagemFundo', function(){
    if(!imagemFundoEndpoint) return;
    const file = $('#modalInputArquivo')[0].files[0];
    const link = $('#modalInputLink').val().trim();
    if(!file && !link){
        alert('Selecione um arquivo ou informe o link.');
        return;
    }
    if(file){
        ...
        $.ajax({
            ...
            success: function(){
                ...
                bootstrap.Modal.getInstance(document.getElementById('modalImagemFundo')).hide();
            },
            error: function(){ alert('Erro ao enviar imagem'); }
        });
    } else {
        $.post(imagemFundoEndpoint, { url: link }, function(resp){
            if(resp.ok){
                alert('Imagem atualizada!');
                bootstrap.Modal.getInstance(document.getElementById('modalImagemFundo')).hide();
            } else {
                alert(resp.msg || 'Erro ao enviar imagem');
            }
        }).fail(function(){ alert('Erro ao enviar imagem'); });
    }
});

window.alterarImagemFundoRifa = function(idRifa) {
    openModalImagemFundo(`/api/rifa/${idRifa}/imagem_fundo`);
}
